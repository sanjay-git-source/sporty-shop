...
if ($result && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $category_display = ($category == 'Female') ? 'Female' : 
                            (($category == 'Male') ? 'Male' : 
                            (($category == 'Kids') ? 'Kids' : $category));

        echo '<tr data-id="' . htmlspecialchars($row['category_id']) . '">
            <td data-label="Sno">' . $n . '</td>
            <td data-label="Name">' . $category_display . '</td>
            <td data-label="Select">
                <input type="checkbox" name="record[]" value="' . htmlspecialchars($row['category_id']) . '">
            </td>
        </tr>';
        $n++;
    }
...
<div id="addNewModal" class="fmodal">
    <div class="fmodal-content">
        ...
        <h3 style="color: #F76100;">Add Category</h3>
        <form id="addFabricForm" action="add_category.php" method="post">
            <select name="category" id="fabric_name" required><br>
...
<script>
    ...
    if (selectedIds.length > 0) {
        if (confirm("Are you sure you want to delete the selected records?")) {
            fetch('delete_categories.php?deleteid=' + selectedIds.join(','), {
                method: 'GET'
            })
