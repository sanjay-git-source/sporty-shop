  ...
  <div class="container">
    ...
    <div class="table-wrapper">
      <table>
        ...
        <tbody>
          <?php 
          $result = $conn->query("SELECT * FROM colors ORDER BY color_id DESC");

          if ($result && $result->num_rows > 0) {
              $n = 1;
              while ($row = $result->fetch_assoc()) {
                  echo '<tr data-id="' . htmlspecialchars($row['color_id']) . '">
                      <td data-label="Sno">' . $n . '</td>
                      <td data-label="Name">' . htmlspecialchars($row['color_name']) . '</td>
                      <td data-label="Select">
                          <input type="checkbox" name="record[]" value="' . htmlspecialchars($row['color_id']) . '">
                      </td>
                  </tr>';
                  $n++;
              }
...
<div id="addNewModal" class="fmodal">
    <div class="fmodal-content">
        ...
        <h2 style="color: #F76100;">Add Color</h2>
        <form id="addFabricForm" action="add_color.php" method="post">
            <label for="fabric_name">Color :</label><br>
            <input type="text" id="fabric_name" name="color" required><br>
...
<script>
    ...
    if (selectedIds.length > 0) {
        if (confirm("Are you sure you want to delete the selected records?")) {
            fetch('delete_colors.php?deleteid=' + selectedIds.join(','), {
                method: 'GET'
            })
